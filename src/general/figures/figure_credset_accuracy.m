...
belowp05 = (actual <= p05)*1;
belowp05(isnan(actual)) = NaN;
belowp05(isnan(p05)) = NaN;
belowp05 = squeeze(mean(mean(belowp05,1,'omitnan'),4,'omitnan'))*100;

abovep95 = (actual >= p95)*1;
abovep95(isnan(actual)) = NaN;
abovep95(isnan(p95)) = NaN;
abovep95 = squeeze(mean(mean(abovep95,1,'omitnan'),4,'omitnan'))*100;

%Plot accuracy for the three forecast horizons
plotops
ploterrs(squeeze(abovep95(1,:,:,:)),[V.yg V.pi V.in],V,'Percent above $5-95/%$ credible set of 1Q ahead forecasts',[savename,'above1Q']);
ploterrs(squeeze(abovep95(2,:,:,:)),[V.yg V.pi V.in],V,'Percent above $5-95/%$ credible set of 4Q ahead forecasts',[savename,'above4Q']);
ploterrs(squeeze(abovep95(3,:,:,:)),[V.yg V.pi V.in],V,'Percent above $5-95/%$ credible set of 8Q ahead forecasts',[savename,'above8Q']);

ploterrs(squeeze(belowp05(1,:,:,:)),[V.yg V.pi V.in],V,'Percent below $5-95/%$ credible set of 1Q ahead forecasts',[savename,'below1Q']);
ploterrs(squeeze(belowp05(2,:,:,:)),[V.yg V.pi V.in],V,'Percent below $5-95/%$ credible set of 4Q ahead forecasts',[savename,'below4Q']);
ploterrs(squeeze(belowp05(3,:,:,:)),[V.yg V.pi V.in],V,'Percent below $5-95/%$ credible set of 8Q ahead forecasts',[savename,'below8Q']);
...
% close all
    ...
    legstrs = {'DGP','NL-PF-5\%','PW-IF-0\%','Lin-KF-0\%'};
