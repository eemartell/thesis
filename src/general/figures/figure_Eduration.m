...
for iZLBdur = ZLBdurs
    ...
    for imethod = 1:3
        temp = Eduration_est(:,:,:,imethod);
        temp = temp(idxs(:));
        qEduration_est(iZLBdur,:,imethod) = [mean(temp,'omitnan'),quantile(temp,[.05,.95])];
    end
end
...
figure(3)
set(gcf,'Position',plotposition);
scatter(zlblasts(:),reshape(Eduration_est(:,:,:,1),npers*nsims*nZLBpers,1))
hold on
plot([0,15],[0,15],'k')
plot([0,14],[1,15],'k')
title('Expected duration of ZLB (number of quarters)','Interpreter','latex','fontsize',10);
xlabel('ZLBlast from PWLinear Algorithm')
ylabel('Simulated PWLinear')

%zlblasts(zlblasts==0) = NaN;

%%
figure(4)
set(gcf,'Position',plotposition);
cdfplot(reshape(Eduration_est(:,:,:,1),npers*nsims*nZLBpers,1)-zlblasts(:))
title('Expected Duration of ZLB (number of quarters)','Interpreter','latex','fontsize',10);
xlabel('PWLinear Simulation - ZLBlast')
