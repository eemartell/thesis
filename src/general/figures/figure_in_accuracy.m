...
in = squeeze(actual(:,V.in,:,:,1));
in_est = squeeze(states(:,V.in,:,:,:));
...
ins = 1:20;
for i_in = ins
    idxs = in(:) < -i_in+1 & in(:) >= -i_in;
    for imethod = 1:2
        temp = in_est(:,:,:,imethod);
        temp = temp(idxs(:));
        qin_est(i_in,:,imethod) = quantile(temp,[.5,.05,.95]);
    end
end
...
for isubplot = 1:2
    ...
    % Shade       
    x=-ins(1:12)+0.5;
    y1 = qin_est(1:12,1,imethod)';
    y2 = qin_est(1:12,2,imethod)';
    y3 = qin_est(1:12,3,imethod)';
    X=[x,fliplr(x)];               
    Y=[y3,fliplr(y2)];             
