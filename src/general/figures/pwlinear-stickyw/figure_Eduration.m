...
specs = {...
    ...
    '../../../Estimation-pwlinear-stickyw/','pwlinear','if','me0','Piecewise Linear, Inversion Filter, ME 0$\%$, Sticky Wages';...
    };
...
legstrs = {'Global-PF-5\%','PwLin-IF-0\%','Linear-KF-0\%','PwLin-IF-0\%-Sticky-Wages'};
...
p1(4) = scatter(Eduration(:),reshape(Eduration_est(:,:,:,4),npers*nsims*nZLBpers,1),'MarkerEdgeColor','k');
plot([0,15],[0,15],'k')
...
% close all
% Spec strings for legend
titles = {'PW-IF-0\%','NL-PF-5\%','Lin-KF-0\%','PW-IF-0\%-Sticky-Wages'};
order = [2,1,3,4];
