...
specs = {...
    ...
    '../../../Estimation-pwlinear-stickyw/','pwlinear-stickyw','if','me0','Piecewise Linear, Inversion Filter, ME 0$\%$, Sticky Wages';...
    };
...
for ifolder = 1:size(specs,1)
    ...
    if exist(filename, 'file') == 2
        ...
        if strcmp(specs{ifolder,2},'pwlinear-stickyw')
            statestemp = reshape(statestemp,[npers,Vnstate+2,nsims,nZLBpers]);
            statestemp = statestemp(:,1:Vnstate,:,:);
        else
...
legstrs = {'NL-PF-5\%','PW-IF-0\%','PW-IF-0\%-Sticky-Wages'};
...
p1(3) = scatter(Pzlb(:),reshape(Pzlb_est(:,:,:,3),npers*nsims*nZLBpers,1),'MarkerEdgeColor','k');
plot([0,1],[0,1],'k')
...
% close all
% Spec strings for legend
titles = {'PW-IF-0\%','NL-PF-5\%','PW-IF-0\%-Sticky-Wages'};
order = [2,1,3];
...
for iPzlb = Pzlbbins
    ...
    for imethod = 1:3
        ...
        temp = temp(idxs(:));
        qPzlb_est(iPzlb,:,imethod) = quantile(temp,[.5,.05,.95]);
    end
end
