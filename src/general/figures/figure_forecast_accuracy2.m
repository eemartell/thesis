...
% forecast(:,:,V.yg,:,:,1) = forecast_dgp(:,:,1,:,:);
% forecast(:,:,V.pi,:,:,1) = forecast_dgp(:,:,2,:,:);
% forecast(:,:,V.i,:,:,1) = forecast_dgp(:,:,3,:,:);
% forecast(:,:,V.in,:,:,1) = forecast_dgp(:,:,4,:,:);
...
plotops
ploterrs(squeeze(rmse(1,:,:,:)),[V.yg V.pi V.i V.in],V,'RMSE of 1Q ahead forecasts',[savename,'1Q']);
ploterrs(squeeze(rmse(2,:,:,:)),[V.yg V.pi V.i V.in],V,'RMSE of 4Q ahead forecasts',[savename,'4Q']);
ploterrs(squeeze(rmse(3,:,:,:)),[V.yg V.pi V.i V.in],V,'RMSE of 8Q ahead forecasts',[savename,'8Q']);
...
%     legstrs = {'DGP','NL-PF-5\%','PW-IF-0\%'};
    ...
    for isubplot = 1:prod(plotdim)
        ...
        p1 = bar(c,squeeze(rmse(istate,2:end,2:end)),'FaceColor','flat');
        %for ip1 = 1:numel(p1)
        %    p1(ip1).CData = ip1;
        %end
