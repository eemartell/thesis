...
specs = {...
    ...
    '../../../Estimation-global/me2/states/','global','pf','me2','Global, Particle Filter, ME 2$\%$';  ...
    '../../../Estimation-global/me5/states/','global','pf','me5','Global, Particle Filter, ME 5$\%$';  ...
    '../../../Estimation-global/me10/states/','global','pf','me10','Global, Particle Filter, ME 10$\%$';  ...
...
% close all
    ...
    legstrs = {'DGP','NL-PF-2\%','NL-PF-5\%','NL-PF-10\%','PW-IF-0\%','Lin-KF-0\%'};
        ...
        if isubplot == 1
            ...
            for i=1:length(p0)  % iterate over number of bar objects
                hT=[hT text(p0(i).XData+offset(i),p0(i).YData,num2str(p0(i).YData.','%.2f'), ...
                              'VerticalAlignment','bottom','horizontalalign','center','fontsize',5.5)];
            end
        else
    ...
    set(leg1,'Position',op + [0.03,.120,0,0]);
