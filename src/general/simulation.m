...
for t = 2:npers
    ...
    [sims(t,V.c,:),pigap(1,1,:),sims(t,V.n,:),sims(t,V.q,:),sims(t,V.ups,:)] = ...
        Fallterp85c_F( ...
            ...
            squeeze(sims(t-1,V.x,:)),squeeze(sims(t-1,V.w,:)),...
            pf.c,pf.pigap,pf.n,pf.q,pf.ups);
    sims(t,V.pi,:) = P.pi*pigap;
    % HH FOC utilization (1)
    sims(t,V.rk,:) = S.rk*exp(P.sigups*(sims(t,V.ups,:)-1));
    % Production function (2)
    sims(t,V.yf,:) = (sims(t,V.ups,:).*sims(t-1,V.k,:)./sims(t,V.g,:)).^P.alpha.*sims(t,V.n,:).^(1-P.alpha); 
    % Utilization definition (3)
    sims(t,V.u,:) = S.rk*(exp(P.sigups*(sims(t,V.ups,:)-1))-1)/P.sigups;
    % Firm FOC capital (4)
    sims(t,V.mc,:) = sims(t,V.rk,:).*sims(t,V.ups,:).*sims(t-1,V.k,:)./(P.alpha*sims(t,V.g,:).*sims(t,V.yf,:));
    % Firm FOC labor (5)
    ...
    % Output definition (7)
    sims(t,V.y,:) = (1-P.varphip*(pigap-1).^2/2 ...
        -P.varphiw*(sims(t,V.wg,:)-1).^2/2).*sims(t,V.yf,:) ...
        -sims(t,V.u,:).*sims(t-1,V.k,:)./sims(t,V.g,:);
    % Output growth gap (8)
