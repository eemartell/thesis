...
nqs = {'0q','6q','30q'};
iZLBpers = [1,2,6];
simstemp = textread('../sims7.txt','%f');
sims = reshape(simstemp,[npers,nvar_cap,nsimsall,nZLBpers]);
...
specs = {...
    ...
    ['../Estimation-global/' me '/'],'global','pf',me;         
    };
...
for inq = 1:numel(nqs)
    % ZLB duration
    nq = nqs{inq};
    iZLBper = iZLBpers(inq);
    disp(['ZLB duration = ' nq])
    ...
    for ifolder = 1:size(specs,1)
        ...
        [draws_all,nzlb,NRMSE,MDD,reject0595,postvar,MSE,bias2,isimdone] = ...
            postdraws_conditional(...
                V,P,F,nsims,specs{ifolder,1},sims(:,:,:,iZLBper),0,iZLBper);
