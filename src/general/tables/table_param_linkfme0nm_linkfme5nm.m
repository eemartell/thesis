...
savename = 'table_param_linkfme0nm_linkfme5nm.tex';
...
specs = {...
    ...
    %'../../Estimation-global-nomiss/me5/','global','pf','me5','No misspecification, Global, Particle Filter, ME 5$\%$';    
    %'../../Estimation-pwlinear-nomiss/','pwlinear','if','me0','No misspecification, Piecewise Linear, Inversion Filter, ME 0$\%$';...
    '../../Estimation-linear-nomiss/me0/','linear','kf','me0','No misspecification, Level Linear, Kalman Filter, ME 0$\%$';...
    '../../Estimation-linear-nomiss/me5/','linear','kf','me5','No misspecification, Level Linear, Kalman Filter, ME 5$\%$';...
    };
...
nrmse = rmse./actual;
sigma_nrmse = reshape(sum(nrmse,1),[nZLBpers,nspecs]);
...
for ispec = 1:nspecs
    ...
    rowlab = cell(2,2);
    rowlab{1,1} = '\midrule $\Sigma$';
    rowlab{2,1} = 'N';
    Tc0 = cell(2,nZLBpers);
    for iZLBper = 1:nZLBpers
        Tc0{1,iZLBper} = ['\scs$[',num2str(sigma_nrmse(iZLBper,ispec),'%.2f'),']$'];
        Tc0{2,iZLBper} = ['\scs$',num2str(N(iZLBper,ispec)),'$'];
    end
    Tc0 = [rowlab,Tc0];
...
TcEnd{2,1} = '\caption{Median, $(5\%,95\%)$ credible sets and $[NRMSE]$ of the mean posterior estimated parameters. $\Sigma$ is the sum of NRMSE.}';
