...
savename = 'table_param_nrmse.tex';
...
specs = {...
    ...
    '../../Estimation-linear/me5/','linear','kf','me5','Lin-KF-$5\%$';...       
    };
...
Tc{3,1} = '\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}l*{6}{c}}\toprule';
...
for ispec = 1:nspecs
    % Sum of NRMSE
    rowlab = cell(1,1);
    rowlab{1,1} = specs{ispec,5};
    Tc0 = cell(1,nZLBpers);
    for iZLBper = 1:nZLBpers
        Tc0{1,iZLBper} = ['$',num2str(nrmse(iZLBper,ispec),'%.2f'),'$'];
    end
    Tc0 = [rowlab,Tc0];

    %Add & between cells and \\ at end or rows
    Tc0(:,1:end-1) = strcat(Tc0(:,1:end-1),' & ');
    Tc0(:,end) = strcat(Tc0(:,end),'\\ [2pt]');
...
TcEnd{2,1} = '\caption{Sum of the $\operatorname{NRMSE}$ across the estimated parameters. Columns denote the ZLB duration in the data.}';
TcEnd{3,1} = '\label{tab:NRMSE}';
TcEnd{4,1} = '\end{table}';
