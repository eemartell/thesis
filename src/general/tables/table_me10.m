...
specs = {...
    ...
    '../../Estimation-linear/me10/','linear','kf','me10','Lin-KF-10$\%$';...
    };
...
for ipar = 1:F.nparam
    ...
    for ispec = 1:nspecs
        ...
        for iZLBper = [1,nZLBpers]
            ...
            Tc1{3,itab} = ['\scs$[',num2str(nrmse(ipar,iZLBper,ispec),'%.3f'),']$'];
            itab=itab+1;
        end
        Tc0 = [Tc0,Tc1];
    end
...
TcEnd{2,1} = '\caption{Median, $(5\%,95\%)$ credible sets and $[NRMSE]$ of the mean posterior estimated parameters. $\Sigma$ is the sum of the NRMSE.}';
