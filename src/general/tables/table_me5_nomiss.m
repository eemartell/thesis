...
savename = 'table_me5_nomiss.tex';
%Specifications to include: path, method, filter, measurement error, table
%label
specs = {...
    '../../Estimation-global-nomiss/me5/','global','pf','me5','NL-PF-5$\%$'; ...   
    '../../Estimation-pwlinear-nomiss/','pwlinear','if','me0','PW-IF-0$\%$';...  
    '../../Estimation-linear-nomiss/me5/','linear','kf','me5','Lin-KF-5$\%$';...   
    };
...
Tc{4,1} = ' & & \multicolumn{6}{c}{No Misspecification: DGP and Estimation Use Small-Scale Model} \\ ';
Tc{5,1} = [' Ptr & Truth & \multicolumn{2}{c}{', specs{1,5},...
        '} &  \multicolumn{2}{c}{', specs{2,5},...
        '} &  \multicolumn{2}{c}{', specs{3,5},'}  \\ \midrule'];
Tc{6,1} = ['  & & ', num2str(nqs(1)),...
        'Q &  ', num2str(nqs(6)), ...
        'Q &  ', num2str(nqs(1)), ...
        'Q &  ', num2str(nqs(6)), ...
        'Q &  ', num2str(nqs(1)), ...
...
for ipar = 1:F.nparam
    ...
    for ispec = 1:nspecs
        ...
        for iZLBper = [1,nZLBpers]
            Tc1{1,itab} = ['$',num2str(pmean(ipar,iZLBper,ispec),nformat{ipar}),'$'];
...
TcEnd{2,1} = '\caption{Average, $(5,95)$ percentiles and $[\operatorname{NRMSE}]$ of the parameter estimates.}';
TcEnd{3,1} = '\label{tab:Mestimates-nomiss}';
TcEnd{4,1} = '\end{table}';
