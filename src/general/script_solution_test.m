...
for inode = 1:G.nodes
    ...
    state = [G.g_gr(inode),G.a_gr(inode),G.mp_gr(inode),G.in_gr(inode),G.c_gr(inode)]; 
...
inp = G.in_gr.^P.rhoi.*(S.i*pf.pigap.^P.phipi.*...
      (G.g_gr.*pf.c./(P.g*G.c_gr)).^P.phiy).^(1-P.rhoi).*exp(G.mp_gr);
locs = find(inp <= 1);
...
%% Save results
if strcmp(saving,'on')
    save('solutions/solution_test.mat','pf','O','P','S','G','V');
end
