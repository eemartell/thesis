function [draws_all,nzlb,NRMSE,MDD,reject0595,postvar,MSE,bias2,isimdone] = ...
    postdraws_conditional(V,P,F,nsims,folder,sims,isimdone,iZLBper)
...
for isim = 1:nsims
    ...
    filename = [folder,'zlb',num2str(iZLBper),'/thin/' num2str(isim) '-MHdraws.txt'];
    if (exist(filename,'file') && isimdone(isim)==1)
        ...
        ndiscarddraws = textread([folder,'zlb',num2str(iZLBper),'/ms/',num2str(isim),'-ndiscarddraws.txt'],'%f'); 
        postlogliks = textread([folder,'zlb',num2str(iZLBper),'/thin/',num2str(isim),'-MHpostlogliks.txt'],'%f');        
