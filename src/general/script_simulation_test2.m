clear
clc

% Set random number seed
mtstream = RandStream('mt19937ar','seed',123456);
RandStream.setGlobalStream(mtstream);

% Load options
load('options')
V.nstate = V.nvar-V.nfore;

% Load Fortran solution
pfc = textread('../data-artificial/pfs/pfc.txt','%f');
pf.c = reshape(pfc,G.griddim);
clear pfc
pfpigap = textread('../data-artificial/pfs/pfpigap.txt','%f');
pf.pigap = reshape(pfpigap,G.griddim);
clear pfpigap
pfn = textread('../data-artificial/pfs/pfn.txt','%f');
pf.n = reshape(pfn,G.griddim);
clear pfn
pfq = textread('../data-artificial/pfs/pfq.txt','%f');
pf.q = reshape(pfq,G.griddim);
clear pfq
pfups = textread('../data-artificial/pfs/pfups.txt','%f');
pf.ups = reshape(pfups,G.griddim);
clear pfups

%% Stochastic steady state
T = 20000;
epsg = zeros(T,1);
epss = zeros(T,1);
epsmp = zeros(T,1);
sims = simulation(pf,P,S,G,V,epsg,epss,epsmp);
stochss = sims(end,:);
format longg
disp(stochss')
% Ergodic distribution
nergdraws = 10000;
nergpers = 500;
epsg = randn(nergpers,nergdraws);
epss = randn(nergpers,nergdraws);
epsmp = randn(nergpers,nergdraws);
ergsims = simulation(pf,P,S,G,V,epsg,epss,epsmp);
ergdraws = squeeze(ergsims(end,:,:));
isnumidx = ones(nergdraws,1);
for idraw = 1:nergdraws
    for istate = 1:V.nstate
        if isnan(ergdraws(istate,idraw))
            isnumidx(idraw) = 0;
            break
        end
    end
end
nergdrawsisnum = sum(isnumidx);
ergdrawsisnum = zeros(V.nstate,nergdrawsisnum);
idrawisum = 0;
for idraw = 1:nergdraws
    if isnumidx(idraw) == 1
        idrawisum = idrawisum + 1;
        ergdrawsisnum(:,idrawisum) = ergdraws(:,idraw);
    end
end

format shortg
% in, c, k, x
disp('Ergodic dist. credible sets of endog. vars')

disp('  notional rate 1/99 sims')
disp(quantile(ergdrawsisnum(V.in,:),[.001,.999]))
disp('  notional rate bounds')
disp([G.inmin,G.inmax])

disp('  consumption 1/99 sims')
disp(quantile(ergdrawsisnum(V.c,:),[.001,.999]))
disp('  consumption bounds')
disp([G.cmin,G.cmax])

disp('  capital 1/99 sims')
disp(quantile(ergdrawsisnum(V.k,:),[.001,.999]))
disp('  capital bounds')
disp([G.kmin,G.kmax])

disp('  investment 1/99 sims')
disp(quantile(ergdrawsisnum(V.x,:),[.001,.999]))
disp('  investment bounds')
disp([G.xmin,G.xmax])

disp('  real wage 1/99 sims')
disp(quantile(ergdrawsisnum(V.w,:),[.001,.999]))
disp('  real wage bounds')
disp([G.wmin,G.wmax])

%% Find simulations with ZLB event(s) that match criteria
T = 120;
nsimstemp = 100;
nsims = 5;
% ZLB event(s) criteria
%   Number of ZLB events
ZLBnevents = [3,4];
%   Duration of ZLB event(s) as percentage of simulation length
ZLBpers = [.05,.05];
nZLBpers = numel(ZLBpers);
sims = NaN(T,V.nvar-V.nfore,nsims,nZLBpers);
for iZLBper = 1:nZLBpers
    % ZLB event(s) criteria
    ZLBnevent = ZLBnevents(iZLBper);
    ZLBnq = ZLBpers(iZLBper)*T;
    disp([num2str(ZLBnevent), ' ZLB events, each ', num2str(ZLBnq), ' quarters long'])
    % Lower bound if no ZLB events
    if ZLBnq == 0
        inbound = 1.00125;
    elseif ZLBnq > 0
        inbound = 1.00;
    end
    tic
    isim = 0;
    while isim < nsims
        % Simulate
        simstemp1 = ergdraws(:,ceil(rand(nsimstemp,1)*nergdraws));
        epsg = randn(T,nsimstemp);
        epss = randn(T,nsimstemp);
        epsmp = randn(T,nsimstemp);
        simstemp = simulation(pf,P,S,G,V,epsg,epss,epsmp,simstemp1);
        
        % Find simulations that satisfy ZLB event(s) criteria
        binds = squeeze(simstemp(:,V.in,:) <= inbound);
        bindsdiff = binds(2:T,:) - binds(1:T-1,:);
        for isimtemp = 1:nsimstemp
            temp = simstemp(:,:,isimtemp);
            isnantemp = any(isnan(temp(:)));
            if ~isnantemp
                nZLBqtemp = sum(binds(:,isimtemp));
                savesim = 0;
                if ZLBnevent == 0
                    savesim = nZLBqtemp == ZLBnq && isim < nsims;
                elseif ZLBnevent >= 1
                    enters = bindsdiff(:,isimtemp) == 1; % use where in Fortran
                    nenters = sum(enters);
                    exits = bindsdiff(:,isimtemp) == -1; % use where in Fortran
                    nexits = sum(exits);
                    if ZLBnevent == 1
                        savesim = ((nenters == ZLBnevent && nexits == ZLBnevent) || ...
                                (nenters == ZLBnevent && nexits == 0) || ...
                                (nenters == 0 && nexits == ZLBnevent)) && ...
                            nZLBqtemp == ZLBnq && ...
                            isim < nsims;
                    elseif ZLBnevent > 1
                        nqs = [];
                        nevents = 0;
                        for t = 1:T-1
                            if bindsdiff(t,isimtemp) == 1
                                first = t;
                            elseif bindsdiff(t,isimtemp) == -1
                                nevents = nevents + 1;
                                nqs(nevents) = t - first;
                            end
                        end
                        savesim = (nenters == ZLBnevent && nexits == ZLBnevent) && ...
                            all(nqs == ZLBnq) && ...
                            isim < nsims;
                    end       
                end
                
                % Save simulation
                if savesim
                    isim = isim + 1;
                    sims(:,:,isim,iZLBper) = simstemp(:,:,isimtemp);
                    disp(isim)
                end
            end
        end
    end
end
toc
binds = squeeze(sims(:,V.in,:,:) <= 1);
bindsdiff = diff(binds,1,1);

%% Test Fotran sims
nsims = 100;
simstemp = textread('../data-artificial/sims.txt','%f');
sims = reshape(simstemp,[120,V.nstate,nsims,6]);
binds = squeeze(sims(:,V.in,:,:) <= 1);
bindsdiff = diff(binds,1,1);

simstemp = textread('../data-artificial2/sims.txt','%f');
sims2 = reshape(simstemp,[120,V.nstate,nsims,2]);
binds2 = squeeze(sims2(:,V.in,:,:) <= 1);
bindsdiff2 = diff(binds2,1,1);

simsdiff = sims(:,:,:,1:3) - sims2;
max(abs(simsdiff(:)))