...
\begin{gather}
  \tilde{V}_{\lambda,t,1} = \beta E_t[(1/\tilde{\lambda}_{t+1})(s_ti_t/(\bar{\pi}\pi^{gap}_{t+1}g_{t+1}))]\\%
  \tilde{V}_{\lambda,t,2} = \beta E_t[(1/\tilde{\lambda}_{t+1})(s_t/(\bar{\pi}\pi^{gap}_{t+1}g_{t+1}))]\\%  
\tilde{V}_{\pi_t,i} = 1 - \theta + \theta\tilde{w}_t + \beta\varphi E_t[(\tilde{\lambda}_t/\tilde{\lambda}_{t+1})(\pi^{gap}_{t+1}-1)\pi^{gap}_{t+1}(\tilde{y}_{t+1}/\tilde{y}_t)]\\
\end{gather}
for $i=1,2$ where $i=1$ corresponds to the non-ZLB regime and $i=2$ corresponds to the ZLB regime.
\begin{gather}
