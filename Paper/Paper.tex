...
ed using \hyperlink{Sims}{\color{black}{Sims's (2002) \texttt{gensys}}} algorithm. (For GHLS, we initialize the regime-indexed policy functions by setting them to the log-linear solution.)
...
\captionsetup{justification=centering}
  ...
  \begin{tabular}{l c c c c c c}
    ...
    \hline
  ART & 2401  & 67 & 17.9s & 78125 & 65 & 0h 15m 37.1s\\    
  GHLS & 2401  & 66 & 46s & 78125 & 1170 &  5h 32m 48.6s\\
  \hline
\end{tabular}
...
The GHLS method is slower for both the model with and without capital. The speed
differential blows up for the model with capital. The performance is somewhat contingent upon the
parameterization. The GHLS method was faster with $\sigma_v = 0.002$, but we selected $\sigma_v = 0.0035$
so that the ZLB constraint was better represented in the simulation. The GHLS solution
algorithm handles a labor policy (consumption policy for model without capital) at and away from
the ZLB. As a result, the solution algorithm requires two interpolation steps. In addition, the GHLS solution algorithm must determine at which nodes the state space is at the ZLB, and
...
computed at and away from the ZLB, and combined later, so all three are shown in the figure. The
GHLS combined policy function is very similar to the policy function away from the ZLB.

Visually, the ART labor policy function is smoother. Labor hours smoothly
...
\captionsetup{justification=centering}
  ...
  \begin{tabular}{l c c c c}
    ...
    \hline
    ART policy & 0.46545\% &0.0020137 $c$  units & 0.19585\% & .0029035 $l$ units \\    
  GHLS policy & 0.45556\% & 0.0019792 $c$ units & 0.56142\% & 0.0076665 $l$ units \\
  \hline
\end{tabular}
...
functions. The mean percent error is calculated as $1/M\sum_{i=1}^N[\textbf{pf.n}(i)-\hat{\textbf{n}}(i)]/\bar{n}$
where $N$ is the
number of periods, $\textbf{pf.n}$ is the labor policy function, $\hat{\textbf{n}}$ is the linear regression model of $\textbf{pf.n}$ fit to
the state variables, and $\bar{n}$ is steady state labor. The corresponding results for the model without
...
capital, both functions are smooth, but the GHLS policy function slightly better fits a linear
plane than the ART policy function.

\begin{figure}[H]
\hypertarget{Figure 2}  
    \centering
\captionsetup{justification=centering}
    \includegraphics[width=16cm,scale=.8]{eeerrorsCAP.pdf}%{pfs3D_CAPITAL.eps}
...
%The GHLS solution is the same as above, except $\textbf{pf}_t(d) \equiv  [\tilde{\pi}^{gap}_t(\textbf{z}_t), n_t(\textbf{z}_t),n_t^{ZLB}(\textbf{z}_t), q_t(\textbf{z}_t), mc_t(\textbf{z}_t)]$ for the model with capital and $\textbf{pf}_t(d) \equiv [\tilde{\pi}^{gap}_t(\textbf{z}_t), \tilde{c}_t(\textbf{z}_t),\tilde{c}_t^{ZLB}(\textbf{z}_t)]$ for the model without capital where the policy functions away and at the ZLB are computed according to (\ref{eq:18}) and (\ref{eq:19}). The initial guess for $n_t^{ZLB}$ is set to the log-linear solution for $n_t$ from \texttt{gensys}. For step 2, in addition to what is in the ART solution method,
%\begin{enumerate}
%\item[(a)] Solve for $i_t$. If $i_t > 1$, solve for all time $t$ variables with $n_t$. Otherwise, solve for all time $t$ variables with $n_t^{ZLB}$. 
%\item[(b)] Linearly interpolate both $n_t$ and $n_t^{ZLB}$ separately, as well as the other policy functions in $\textbf{pf}_{j-1}$ atF the updated state variables. 
%\item[(c)] Solve for $\textbf{i}_t$ using the interpolated policy functions and time $t$ variables. If the node $\textbf{i}_t(d) > 1$, assign $\textbf{n}_t^{combined}(d)$ to the value in $\textbf{n}_t(d)$, otherwise $\textbf{n}_t^{combined}(d)$ to the value in $\textbf{n}_t^{ZLB}(d)$. Solve for the other elements in $\textbf{s}_{t+1}$ and approximate the expectation operators.
%\item[(d)] Using the expectation operators, solve for the $n_t(d)$ policy in the style of (\ref{eq:20}) and the $n_t^{ZLB}$ policy in the style of (\ref{eq:21}). Solve for the rest of the policy functions in $\textbf{pf}_t(d)$.  
%\end{enumerate}

\section{Results for model without capital} \hypertarget{Appendix C}
The following reports the corresponding results for the model wtihout capital. 
...
ZLB. The average percent error of the consumption policy functions from a linear plane is slightly
lower for the GHLS policy functions.
...
logarithms for the consumption Euler equation and the price Phillips curve. The mean and maximum Euler equations are comparable between algorithms; the GHLS solution does slightly
better for the consumption Euler equation and the ART solution does slightly better for the
