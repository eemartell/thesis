...
\hyperlink{Table 2}{Table 2} reports the solution times for the Richter et al. (2013) and Gust et al. (2017) solution methods for the models with and without capital. The solution times were computed with multi-core processing using the Parallel Computing Toolbox and integrate Fortran in the interpolation steps of our algorithm through MATLAB executable functions. The solution times represent one run of each algorithm. The machine used to compute solution times has two CPUs (at 2.30GHz), each with 16 cores. %Say something about the solution times%
\begin{itemize}
